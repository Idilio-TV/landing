import { supabase, ShowWithEpisodes, Show, Content, Season } from './supabase'
...
export async function getShowIdsForStaticGeneration(): Promise<string[]> {
  ...
  return data?.map((show: { id: string }) => show.id) || []
}
...
export async function getAllShowsWithPosters(): Promise<(Show & { poster?: Content })[]> {
  ...
  const { data: posters, error: postersError } = await supabase
    ...
    .eq('content_type', 'poster')
    .is('deleted_at', null)
    .in('show_id', shows.map((show: Show) => show.id))
  ...
  return shows.map((show: Show) => ({
    ...show,
    poster: posters?.find((poster: Content) => poster.show_id === show.id)
  }))
}
